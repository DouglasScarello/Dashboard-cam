...
| :--- | :--- | :--- | :--- | :--- |
| `2026-08-31T03:15:00-03:00` | `[FEAT]` | `olho_de_deus/camera_grid_server.py`, `db_manager.py`, `schema.sql`, `scratch/migrate_json_to_sqlite.py` | Exposição da fonte de ingestão via API: coluna `source` (existia só no JSON, nunca migrada pro SQLite) adicionada ao schema; novo filtro `?source=` em `GET /api/cameras`; novos endpoints `GET /api/metadata/sources` (contagem por fonte) e `GET /api/metadata/stats` (resumo: total/online/offline/top países/áreas/fontes). | Claude (sessão autônoma noturna) |
| `2026-08-31T02:00:00-03:00` | `[FIX]` | `olho_de_deus/camera_grid_server.py`, `hls_liveness.py`, `content_validation.py` | 4 correções reais da classe "suposição desatualizada pré-migração": (1) `_cameras` indefinida no endpoint de snapshot forense (`NameError` garantido em qualquer id desconhecido); (2) `hls_liveness.py` rodava validação de manifesto `.m3u8` contra câmeras `SNAPSHOT_JPEG`, sobrescrevendo o estado correto delas; (3) inferência de `video_id` via `"v=" in url` sem checar `stream_format`/domínio, podia sequestrar câmera não-YouTube pro player errado; (4) `content_validation.py` rodava yt-dlp contra o catálogo inteiro (6281 câmeras) mesmo com 0 câmeras YouTube restantes no dataset. | Claude (sessão autônoma noturna) |
| `2026-08-31T00:40:00-03:00` | `[FEAT]` | `olho_de_deus/ingest_ontario511.py`, `ingest_nzta.py`, `ingest_iceland.py`, `snapshot_liveness.py`, `SnapshotImagePlayer.tsx` | Novo tipo de câmera `SNAPSHOT_JPEG` (imagem única por request, sem stream contínuo) suportado de ponta a ponta (schema → migração → API → tipo TS → player React). +1585 Ontario 511, +319 NZTA, +500 Islândia/Vegagerdin — todas sem necessidade de cadastro/API key, todas com liveness real validada (magic bytes JPEG, não só HTTP 200). Pesquisa completa de ~15 fontes adicionais documentada em `NOITE_AUTONOMA_2026-08-30.md`. | Claude (sessão autônoma noturna) |
