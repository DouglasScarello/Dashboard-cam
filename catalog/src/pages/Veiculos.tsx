...
import { Car, X, MapPin, Clock } from 'lucide-react';
...
interface PlateRead {
    ...
    plate_evidence_path: string | null;
...
export default function Veiculos() {
    ...
    useEffect(() => {
        let mounted = true;
        fetch(`${API_BASE}/api/vehicles?limit=200`)
            .then(res => {
                if (!res.ok) throw new Error(`HTTP ${res.status}`);
                return res.json();
            })
            .then((data: VehicleRow[]) => {
                if (!mounted) return;
    ...
    useEffect(() => {
        rows.forEach(v => {
            if (photos[v.id]) return;
            fetch(`${API_BASE}/api/vehicles/${v.id}`)
                .then(res => res.json())
                .then((detail: VehicleDetail) => {
                    const last = detail.history?.[detail.history.length - 1];
                    if (last?.evidence_url) {
                        setPhotos(prev => ({ ...prev, [v.id]: `${API_BASE}${last.evidence_url}` }));
                    }
                })
                .catch(() => {});
        });
        ...
        <main className="flex-1 px-8 py-6 overflow-y-auto custom-scrollbar">
            <div className="flex items-center justify-between gap-4 mb-6 pb-4 border-b border-white/5">
                <div>
                    <h2 className="text-sm font-black tracking-widest uppercase text-white flex items-center gap-2">
                        <Car className="w-4 h-4 text-accent-emerald" /> VEÍCULOS CATALOGADOS
                    </h2>
                    <p className="text-[10px] font-mono text-accent-emerald tracking-wider uppercase mt-1">
                        {rows.length} REGISTROS — PLACA, COR E CARROCERIA POR RECONHECIMENTO
                    </p>
                </div>
            </div>
            ...
            {loading ? (
                ...
                <div className="text-muted italic text-sm">
                    Nenhum veículo catalogado ainda. Assim que uma placa fechar consenso numa câmera monitorada, aparece aqui sozinho.
                </div>
            ) : (
                    ...
                    {rows.map(v => (
                        ...
                        >
                            <div className="aspect-video relative bg-neutral-900 overflow-hidden">
                                {photos[v.id] ? (
                                    <img src={photos[v.id]} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" alt={v.plate_text} />
                                ) : (
                                ...
                                {v.times_seen > 1 && (
                                    <div className="absolute top-3 right-3 flex items-center gap-1.5 bg-accent-emerald/80 backdrop-blur-sm px-2 py-1 rounded-full">
                                        <span className="text-[9px] font-black tracking-widest text-black uppercase">{v.times_seen}× VISTO</span>
                                    </div>
                                )}

                                <div className="absolute bottom-3 left-3 right-3 glass-panel rounded-lg border border-white/10 px-3 py-2.5">
                                    <div className="flex items-center justify-between gap-1 mb-1">
                                        <h3 className="text-xs font-black uppercase tracking-tight text-white font-mono">{v.code || 'V-?????????-?'}</h3>
                                        {v.country_code && (
                                            <span className="text-[8px] font-mono px-1.5 py-0.5 rounded bg-white/10 text-accent-amber shrink-0 uppercase">{v.country_code}</span>
                                        )}
                                    </div>
                                    <p className="text-[10px] text-accent-amber font-mono truncate">{v.plate_text}</p>
                                    <p className="text-[9px] text-muted font-mono mt-0.5 truncate">{v.color || '?'} {v.body_type || ''}</p>
                                </div>
                            </div>
                        </motion.div>
                    ))}
            ...
            <AnimatePresence>
                {selectedId != null && (
                    <VehicleDossier id={selectedId} onClose={handleClose} />
                )}
            </AnimatePresence>
        </main>
    );
...
function VehicleDossier({ id, onClose }: { id: number; onClose: () => void }) {
            ...
            >
                <div className="h-16 px-6 border-b border-white/5 flex items-center justify-between bg-white/[0.02]">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-[10px] font-black tracking-widest text-muted uppercase font-mono">{!detail ? 'CARREGANDO…' : (detail.code || 'V-?????????-?')}</span>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar flex flex-col md:flex-row">
                        <aside className="w-full md:w-64 border-r border-white/5 p-6 flex flex-col gap-4 shrink-0">
                            ...
                            <div className="text-xs text-white/70 space-y-1.5 font-mono">
                                <p>Placa <b className="text-accent-amber">{detail.plate_text}</b> ({detail.country_code || '?'})</p>
                                <p>{detail.color || '?'} {detail.body_type || ''}</p>
                                ...
                                <div className="space-y-3">
                                    {history.slice().reverse().map(h => (
                                        <div key={h.id} className="p-3 bg-white/[0.03] border border-white/5 rounded-xl flex items-center gap-3">
                                            {/* Duas fotos: veículo inteiro (contexto/cor/carroceria) e o
                                                recorte só da placa (o que realmente fechou o OCR) — pedido
                                                do usuário pra poder ver as duas, não só uma foto genérica. */}
                                            <div className="flex gap-1.5 shrink-0">
                                                {h.evidence_url ? (
                                                    <img src={`${API_BASE}${h.evidence_url}`} title="Veículo completo" className="w-14 h-14 object-cover rounded-lg border border-white/10" />
                                                ) : (
                                                    <div className="w-14 h-14 rounded-lg bg-black/40" />
                                                )}
                                                {h.plate_evidence_url ? (
                                                    <img src={`${API_BASE}${h.plate_evidence_url}`} title="Placa (recorte)" className="w-14 h-14 object-contain bg-black rounded-lg border border-accent-amber/30" />
                                                ) : (
                                                    <div className="w-14 h-14 rounded-lg bg-black/20 border border-dashed border-white/10 flex items-center justify-center">
                                                        <span className="text-[7px] text-muted font-mono text-center leading-tight">sem<br/>recorte</span>
                                                    </div>
                                                )}
                                            </div>
                                            <div className="text-xs">
                                                ...
                                                <p className="text-muted font-mono mt-0.5">
                                                    {fmtTime(h.created_at)} · confiança {Math.round((h.confidence || 0) * 100)}% · {h.frames_voted ?? '?'} frames
                                                </p>
                                            </div>
                                        </div>
                                    ))}
